...
<body>
  ...
  if ($producto=="Comestible") {
    ...
    FROM Comestibles as c AND Productos as p AND carritos as ca AND Compras as co AND Tiendas as t
    WHERE c.id = p.id AND p.id = ca.id_productos AND ca.id_compras = co.id AND co.id_tienda = t.id
    GROUP BY t.nombre;";
  }
  if ($producto == "Comestible Frescos") {
    $query = "SELECT t.nombre, COUNT(c.id)
    FROM Frescos as f AND Comestibles as c AND Productos as p AND carritos as ca AND Compras as co AND Tiendas as t
  ...
  if ($producto == "Comestible Conserva") {
    $query = "SELECT t.nombre, COUNT(c.id)
    FROM Conserva as cons AND Comestibles as c AND Productos as p AND carritos as ca AND Compras as co AND Tiendas as t
    WHERE f.id = c.id AND c.id = p.id AND p.id = ca.id_productos AND ca.id_compras = co.id AND co.id_tienda = t.id
    GROUP BY t.nombre;";
  }
  ...
  if ($producto == "No Comestible") {
    $query = "SELECT t.nombre, COUNT(c.id)
    FROM No_Comestibles as c AND Productos as p AND carritos as ca AND Compras as co AND Tiendas as t
    WHERE c.id = p.id AND p.id = ca.id_productos AND ca.id_compras = co.id AND co.id_tienda = t.id
    GROUP BY t.nombre;";
  }
